...
module.exports = {
  ...
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    ...
    let { message: { embeds }, user: { tag, id, accentColor } } = interaction, { footer: { text } } = embeds[0], currentPageIndex = parseInt(text), nextPageIndex = currentPageIndex - 1, perPage = 25, firstServerIndex = perPage * nextPageIndex - perPage
    ...
    let serverSelectRow = new ActionRowBuilder().addComponents(serverSelect), buttonRow = undefined

    //Check if previous page button is required
    if (nextPageIndex > 1) {
      buttonRow = new ActionRowBuilder().addComponents(previousPageButton)
    }

    //Check if next page button is required
    if (userServers.length > (nextPageIndex * perPage)) {
      buttonRow == undefined ? buttonRow = new ActionRowBuilder().addComponents(nextPageButton) : buttonRow.addComponents(nextPageButton)
    }
