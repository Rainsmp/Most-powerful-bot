    ...
    await interaction.editReply({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(`${await emojiManager.getEmoji("emoji_arrow_down_right")} **${await t("server_manager_events.reinstall_server_text")}**`)
          .setColor(accentColor ? accentColor : 0xe6b04d)
          .setFooter({ text: process.env.FOOTER_TEXT, iconURL: serverIconURL })
          .setTimestamp()
      ],
     ...
     //Logging
     await logManager.logString(`${tag} reinstalled his Server with the identifier ${identifier}`)
     return;
   },
 };
